 ...
 *
 * Throws if a container's `class_name` matches no Durable Object this script
 * implements. Returns `undefined` if every container resolves only to a
 * cross-script binding, since those are owned by another Worker.
 */
function buildPreviewContainerConfig(
	...
	// A container whose `class_name` matches no Durable Object at all is a
	...
	for (const container of previewContainers) {
		if (
			ownedDOClasses.has(container.class_name) ||
			previewDOBindings.some((b) => b.class_name === container.class_name)
		) {
			continue;
		}
		throw new UserError(
			`The container class_name "${container.class_name}" in "previews.containers" does not match any Durable Object class in your ${configFileName(config.configPath)} file. Declare the class in "migrations" or "exports", or bind it under "previews.durable_objects".`,
			{
	...
	const filteredContainers = previewContainers
		.filter((c) => ownedDOClasses.has(c.class_name))
		.map((c) => ({
			...c,
			name:
				c.name ??
