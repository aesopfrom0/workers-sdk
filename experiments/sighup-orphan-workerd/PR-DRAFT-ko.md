...
**호스트 프로세스에만** 신호를 보낸다 — workerd 가 직접 받으면 스스로 죽어서 측정이 무의미해진다.
조합당 3회, GitHub 러너, miniflare 4.20260701.0:
...
별도로 `wrangler dev` 경로도 같은 방식으로(workerd 를 소유한 프로세스에만 신호) 돌렸고
양쪽 OS 에서 같은 결과가 나왔다. SIGTERM/SIGINT 는 수정 전후 모두 정상 = **회귀 없음.**
앞선 macOS 30회도 같은 결과.
...
## ✅ 2026-08-17 해결 — 표와 본문의 모순 제거

**문제였던 것**: 본문은 "`wrangler dev` 는 같은 그룹이라 안 샌다"고 하는데
표는 "Linux `wrangler dev` — 고아 3/3" 이라 **모순처럼 읽혔다.**
(실제로는 표 데이터가 "부모에게만 신호를 보낸" 인위적 조건이라 안 모순이었지만, 그게 안 드러남)

**해결**: 임베드 케이스를 CI 에 추가하고(`embed-trial.mjs`) 표를 그 결과로 교체.
이제 **본문의 주장과 표가 같은 것을 가리킨다.**
...
원자료: `~/z-idea-brewery/dev-docs/wrangler-orphan-evidence/results-ci-embed.tsv`

`wrangler dev` 경로 결과(54회)는 **회귀 없음 근거**로 한 문단에 요약만 남겼다.

## 남은 판단

1. **Windows 검증 없이 지금 낼지** — 주 근거가 "임베드"로 확정되면서 Windows 창 닫기 검증의
   가치가 더 떨어졌다. 게다가 **임베드 경로는 Windows 에서 측정 자체가 불가능**하다
   (`process.kill` 로 SIGHUP 전달 불가). 지금 내도 잃는 게 거의 없다는 판단.
2. **길이** — 표를 줄이면서 오히려 짧아졌다. 아래 참조.
